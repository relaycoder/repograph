...
import type { CodeNode, CodeNodeType, CodeNodeVisibility, FileContent } from '../types';

// --- UTILITY FUNCTIONS ---

export const getNodeText = (node: TSNode, content: string): string => content.slice(node.startIndex, node.endIndex);
export const getLineFromIndex = (content: string, index: number): number => content.substring(0, index).split('\n').length;

export const extractCodeSnippet = (symbolType: CodeNodeType, node: TSNode): string => {
...
export const extractQualifiers = (childCaptures: TSMatch[], fileContent: string, handler: Partial<LanguageHandler>) => {
...
export const getCssIntents = (ruleNode: TSNode, content: string): readonly ('layout' | 'typography' | 'appearance')[] => {
...
export const getLangHandler = (langName: string): LanguageHandler => ({ ...languageHandlers['default'], ...languageHandlers[langName] } as LanguageHandler);

export function getSymbolTypeFromCapture(captureName: string, type: string): CodeNodeType | null {
...
export function findEnclosingSymbolId(startNode: TSNode, file: FileContent, nodes: readonly CodeNode[]): string | null {
...
export function processSymbol(context: ProcessSymbolContext, langConfig: LanguageConfig): void {
